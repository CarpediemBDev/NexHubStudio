 ...
 *
 * [주의] 화면은 더 이상 이 함수를 부르지 않는다.
 * 판정의 권위는 백엔드에 있고(RegulationConflictEngine.java), 화면은 실시간 예측도
 * POST /regulations/detect-conflicts 로 받는다. 저장할 때 서버가 다시 판정하므로
 * 화면이 따로 판정해 봐야 결과가 갈라질 뿐이다.
 *
 * 여기 남은 detectConflicts 를 쓰는 곳은 목업 핸들러 하나다 —
 * 핸들러가 백엔드 대역이므로 같은 규칙이 필요하고, 그 규칙의 원본이 이 파일이다.
 * 판정 규칙을 고치면 자바 쪽도 같이 고칠 것.
 *
 * codeName / targetCodes / itemCodes 는 화면이 계속 쓴다(표시용).
 *
