...
3. 그래서 **옵션을 false 로 하면 `rg-alternate-row` 클래스 자체가 안 붙고**, CSS가 칠할 대상이 없어져 단일색이 된다.
   No. 칸도 같은 옵션을 읽으므로 `rg-alternate-row-bar` 도 같이 빠진다.

> ⚠ **데이터 영역과 No. 칸은 클래스 이름이 다르다.**
>
> ```
> 데이터 행:  <tr class="rg-data-row rg-alternate-row">      (.rg-body 안)
> No. 칸 행:  <tr class="rg-row-bar rg-alternate-row-bar">   (.rg-rowbarcontainer 안)
> ```
>
> RealGrid 기본 테마 CSS 7종에는 `rg-alternate-row-bar` 규칙이 없어서 기본 상태의 No. 칸은 줄무늬가 없다.
> 하지만 **프로젝트 자체 테마·공통 CSS가 No. 칸 행을 칠하는 경우가 있다.** 이때 `.rg-alternate-row` 만 덮으면
> **No. 칸만 흰색·회색 줄무늬로 남는다.** CSS로 처리할 땐 두 클래스를 항상 같이 다룬다 (4장).
...
> - No. 칸 (`/grid-studio/js`, 20행): 적용 전 `tr.rg-alternate-row-bar` 5개 → 적용 후 **0개**
...
```css
/* 홀수 행 배경을 짝수 행과 같게 — 데이터 영역 + No./체크 칸 */
.rg-root .rg-body .rg-alternate-row,
.rg-root .rg-rowbarcontainer .rg-alternate-row-bar {
  background: none;
}
```

- **두 번째 줄(No. 칸)을 빼먹지 않는다.** 빼면 프로젝트 CSS가 No. 칸을 칠하고 있을 때 No. 칸만 줄무늬가 남는다 (2장 ⚠).
- 선택자를 기본 규칙(`.rg-alternate-row`)보다 **구체적으로** 써야 한다. 그래야 CSS 로드 순서와 상관없이 이긴다.
- 줄무늬가 `tr` 이 아니라 `td` 에 칠해져 있다면(`... .rg-alternate-row-bar td { ... }` 같은 규칙) 이 CSS로는 안 지워진다. 7장 방법으로 실제 규칙을 찾아 그 선택자를 덮는다.
...
| CSS로 `.rg-alternate-row` 만 덮음 → **No. 칸만 줄무늬가 남음** | No. 칸 행은 클래스가 `rg-alternate-row-bar` 라 안 걸린다. 4번처럼 두 클래스를 같이 덮거나, 1번(옵션)으로 둘 다 끈다 |
| 옵션을 껐는데도 **No. 칸만 줄무늬가 남음** | 클래스가 아니라 `tr:nth-child(even)` 같은 **순번 선택자**로 칠하는 공통 CSS가 있는 경우. 옵션과 상관없이 칠해진다. 데이터 셀은 다른 규칙이 흰색으로 덮고 있어서 No. 칸에서만 드러난다. 7장 방법으로 그 규칙을 찾아 고친다 |
...
```js
document.querySelectorAll('.rg-alternate-row, .rg-alternate-row-bar').length
```

- `0` → 적용 성공 (1번 방식)
- `0` 보다 크면 → 옵션이 안 들어갔거나, 옵션 설정 이후 다시 그리지 않은 것. `gridView.displayOptions.useAlternateRowStyle` 값과 `refresh(true)` 호출 여부 확인

4번(CSS) 방식을 썼다면 클래스는 남아 있으니 배경색으로 확인 — **No. 칸도 같이 본다**:

```js
const bg = sel => [...document.querySelectorAll(sel)].slice(0, 4)
  .map(el => getComputedStyle(el).backgroundColor)
console.log('데이터 tr', bg('.rg-body .rg-data-row'))
console.log('No. 칸 tr', bg('.rg-rowbarcontainer tbody tr'))
console.log('No. 칸 td', bg('.rg-rowbarcontainer tbody tr td:first-child'))
// 각 줄의 네 값이 모두 같으면 성공.
// 단, 셀(td)에 불투명 배경을 칠하는 프로젝트는 tr 이 줄무늬여도 화면엔 안 보인다 → 눈에 보이는 색은 td 줄로 판단
```

### No. 칸만 줄무늬가 남을 때 — 칠하는 규칙 찾기

No. 칸 두 번째 행(홀수 행)과 그 첫 칸에 걸린 배경 규칙을 전부 찍는다.

```js
const tr = document.querySelectorAll('.rg-rowbarcontainer tbody tr')[1]
console.log('tr class:', tr.className)
for (const s of document.styleSheets) {
  let rules; try { rules = s.cssRules } catch { continue }
  for (const r of rules) {
    if (!r.selectorText || !/background/.test(r.cssText)) continue
    for (const el of [tr, tr.cells[0]]) {
      if (el.matches(r.selectorText)) console.log(s.href || '(inline)', '→', r.cssText)
    }
  }
}
```
...
| 결과 | 원인 · 조치 |
|---|---|
| `tr class` 에 `rg-alternate-row-bar` 가 있음 | 옵션이 안 들어갔거나 다시 안 그림 → 옵션 값·`refresh(true)` 확인 |
| `rg-alternate-row-bar` 를 쓰는 규칙이 찍힘 | 프로젝트 CSS가 No. 칸 줄무늬를 칠함 → 1번(옵션) 또는 4번 CSS의 두 번째 줄 |
| `nth-child` / `nth-of-type` 규칙이 찍힘 | 순번 선택자 → 옵션으로 안 꺼진다. 그 규칙에서 RealGrid 표를 뺀다 (예: `table tr:nth-child(even)` → `table:not(.rg-table) tr:nth-child(even)`) |
| 아무것도 안 찍힘 | `@media` 등 중첩 규칙 안에 있으면 이 스크립트가 못 찾는다 → 개발자도구 Elements 에서 No. 칸 `td` 선택 후 Styles 패널의 `background` 출처 확인 |
